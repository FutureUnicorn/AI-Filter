# Official pinned MinIO artifacts replace the unavailable Quay images.
# Both downloads are checked against checksums published with their releases.
FROM alpine:3.23.3 AS server-amd64
...
FROM alpine:3.23.3 AS server-arm64
...
FROM alpine:3.23.3 AS client-amd64
...
FROM alpine:3.23.3 AS client-arm64
...
ARG TARGETARCH
FROM client-${TARGETARCH} AS client
