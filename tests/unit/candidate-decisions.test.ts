...
// `assert.equal` carries an assertion signature in the pinned @types/node, so
// asserting on `status.status` narrows the union at the call site. The
// defensive checks that followed those assertions were therefore unreachable
// or always-true, and failed typecheck:tests as comparisons with no overlap.
// The unreachable throw was removed and the conditional assertions were
// unwrapped so they run unconditionally, which is stronger than guarding them
// with a condition TypeScript has proven cannot be false. The remaining guard
// inside the CANDIDATE_DECISION_KINDS loop has no narrowing assertion before
// it and still does real work.
