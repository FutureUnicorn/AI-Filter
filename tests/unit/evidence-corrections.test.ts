...
// EvidenceOutcome kind. Attributed here rather than loosening the type.
const SUBJECT = {
...
function supported(criterionId: string): EvidenceOutcome {
  return { ...SUBJECT, schemaVersion: CONTRACT_SCHEMA_VERSION, kind: "supported", criterionId, citation };
}
function notFound(criterionId: string): EvidenceOutcome {
  return { ...SUBJECT, schemaVersion: CONTRACT_SCHEMA_VERSION, kind: "not_found", criterionId };
}
